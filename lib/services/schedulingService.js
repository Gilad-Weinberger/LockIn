...
export const scheduleTasksService = async (userId, forceReschedule = false) => {
  ...
  try {
    ...
    // Create AI scheduling prompt (only for non-events)
    const prompt = createSchedulingPrompt(
      userSchedulingRules,
      nonEventsToSchedule
    );
    ...
    if (nonEventsToSchedule.length > 0) {
      const result = await processTasksWithAI(nonEventsToSchedule, prompt);
    ...
    const aiTasksWithScheduling = aiScheduledTasks
      .map((aiTask) => {
        const originalTask = nonEventsToSchedule.find(
    ...
    return {
      message: `Successfully scheduled ${allScheduledTasks.length} tasks (${preScheduledEvents.length} events with fixed dates + ${aiTasksWithScheduling.length} AI-optimized tasks)`,
