...
/**
 * Scheduling service - schedules tasks for a user using AI
...
export const scheduleTasksService = async (userId, forceReschedule = false) => {
  try {
    ...
    // Check if Gemini API key is available
    if (!process.env.GEMINI_API_KEY) {
      throw new Error("AI service not configured - GEMINI_API_KEY is required");
    }
    ...
    console.log(
      ...
      userSchedulingRules ? "Found custom rules" : "Using default rules"
    );
    ...
    console.log(`${tasksToSchedule.length} tasks need AI scheduling`);
    ...
    if (tasksToSchedule.length === 0) {
      return {
        ...
        tasks: [],
        reasoning: {
          overall_reasoning: "No tasks required scheduling",
          scheduling_summary: {
            total_tasks_scheduled: 0,
            total_time_allocated: "0 hours",
            scheduling_period: "None",
            key_considerations: ["No tasks to schedule"],
          },
        },
      };
    }
