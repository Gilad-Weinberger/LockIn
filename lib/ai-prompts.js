...
export const SCHEDULING_PROMPTS = {
  free: {
    createPrompt: (userSchedulingRules, tasksToSchedule) => {
      const userRulesSection = userSchedulingRules
        ? `USER'S CUSTOM SCHEDULING RULES:
${userSchedulingRules}

CRITICAL: The user's custom scheduling rules above are MANDATORY and must override any default scheduling logic.`
        : `No custom scheduling rules provided. Use basic time management principles.`;
      ...
      return `You are a scheduling assistant focused on organizing tasks efficiently.
...
- Only adjust the time of day, never the date
- Preserve existing times when specified

**Time Estimation:**
- Short tasks: 30 minutes - 1 hour
...
- No weekend work unless specified

**Priority Scheduling:**
- DO tasks: Schedule first, during morning hours
- PLAN tasks: Schedule when time is available
- DELEGATE tasks: Schedule during collaborative hours

**Basic Constraints:**
- No overlapping tasks
- 15-minute buffer between tasks
- Maximum 6 hours of scheduled work per day

For each task, provide:
- Start and end times
- Brief reasoning for timing choice
  ...
  "scheduledTasks": [
    {
      ...
      "reasoning": "Scheduled during morning hours for high focus"
    }
  ]
}`;
    },
  ...
  "scheduledTasks": [
    {
      ...
      "startDate": "2024-01-15T09:00:00.000Z",
      "endDate": "2024-01-15T10:30:00.000Z",
      "reasoning": "Scheduled during peak cognitive hours (9-11 AM) for complex problem-solving task. Duration based on estimated complexity and includes 15-minute buffer for context switching."
    }
  ],
  "reasoning": {
    "overall_reasoning": "Strategic scheduling approach focusing on energy optimization and deadline management",
    "scheduling_summary": {
      "total_tasks_scheduled": 5,
      "total_time_allocated": "6 hours 30 minutes",
      "scheduling_period": "Next 3 days",
      "key_considerations": ["Peak energy utilization", "Deadline prioritization", "Context switching minimization"]
    }
  }
}`;
    },
 ...
 * @param {string} subscriptionLevel - 'free' or 'pro'
 * @param {string} userSchedulingRules - User's custom rules
...
export const getSchedulingPrompt = (
  ...
  const promptType = subscriptionLevel === "pro" ? "pro" : "free";
  return SCHEDULING_PROMPTS[promptType].createPrompt(
    userSchedulingRules,
    tasksToSchedule
  );
};
