...
/**
 * Check if user has shown pricing
 * @param {string} userId - User ID
 * @returns {Promise<boolean>} True if user has shown pricing
 */
export const hasShownPricing = async (userId) => {
  if (!userId) return false;

  try {
    const userData = await getUserData(userId);
    return userData?.pricingShown === true;
  } catch (error) {
    console.error("Error checking pricing shown:", error);
    return false;
  }
};
