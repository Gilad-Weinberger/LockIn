...
/**
 * Sync task to Google Calendar via API route
 * @param {Object} task - Task object
 * @param {string} userId - User ID
...
const syncTaskToGoogleCalendar = async (task, userId, operation) => {
  try {
    // Only run on client-side to avoid server-side fetch issues
    if (typeof window === "undefined") {
      return { success: false, error: "Sync only available on client-side" };
    }
 ...
 */
// This function has been moved to the new Google Calendar integration utility
