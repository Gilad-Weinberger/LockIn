...
export const updateTasksWithScheduling = async (scheduledTasks) => {
  ...
  return handleAsyncOperation(async () => {
    const bulkOps = scheduledTasks.map((task) => ({
      updateOne: {
        filter: { _id: task.id },
        update: {
...
export const updateNullPriorityTasks = async (userId) => {
  return handleAsyncOperation(async () => {
    ...
    const bulkOps = tasksWithNullPriority.map((task, index) => ({
      updateOne: {
        filter: { _id: task._id },
        update: {
          priority: "plan",
...
export const checkNullPriorityTasks = async (userId) => {
  return handleAsyncOperation(async () => {
    ...
    const nullPriorityCount = await Task.countDocuments({
      userId,
      priority: null,
      isDone: false,
    });

    const nullInGroupRankCount = await Task.countDocuments({
      userId,
      inGroupRank: null,
      isDone: false,
      priority: { $in: ["do", "plan"] },
