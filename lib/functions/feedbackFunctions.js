...
// Delete feedback (by the user who created it or by admin)
export const deleteFeedback = async (feedbackId, userId, isAdmin = false) => {
  try {
    // Validate that user is logged in
    if (!userId || userId === "anonymous") {
      return {
    ...
    // Check if the user is the owner of the feedback OR if they are an admin
    if (feedbackData.userId !== userId && !isAdmin) {
      return {
        success: false,
        error: isAdmin
          ? "Admin permissions required to delete this feedback"
