...
import { updateUserApi } from "@/lib/functions/dbApi";
...
export const incrementAIPrioritizationTokens = async (userId, count = 1) => {
  ...
  const currentCount = userData.aiPrioritizationTokensUsed || 0;

  await updateUserApi(userId, {
    aiPrioritizationTokensUsed: currentCount + count,
    updatedAt: new Date(),
  });
};
...
export const incrementAISchedulingTokens = async (userId, count = 1) => {
  ...
  const currentCount = userData.aiSchedulingTokensUsed || 0;

  await updateUserApi(userId, {
    aiSchedulingTokensUsed: currentCount + count,
    updatedAt: new Date(),
  });
};
